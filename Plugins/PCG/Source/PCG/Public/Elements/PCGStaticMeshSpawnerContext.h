...
struct FPCGInstancesAndWeights
{
	...
	/** Local transform to apply after point placement for the matching weighted mesh entry. */
	TArray<FTransform> MeshTransformOverrides;
};
...
#endif
{
	...
	TArray<int> CumulativeWeights;
	/** Local transform to apply after point placement for the matching weighted mesh entry. */
	TArray<FTransform> WeightedMeshTransformOverrides;
	...
	TSet<TWeakObjectPtr<UPCGManagedISMComponent>> TouchedResources;
	...
	{
		bool bMergedResources = false;
		TSet<TWeakObjectPtr<UPCGManagedISMComponent>> Resources;
	};
	
	// information about all the resources used by this spawner for this execution
	FResourceReuseInfo UsedResources;

	// information about all the resources used per input (data crc) by this spawner for this execution
	TMap<uint32, FResourceReuseInfo> DataCrcToUsedResources;
