...
namespace PCGLog
{
	 ...
	 * Note that if the context is not set, it will look for the current context for this thread.
	 */
	...
	namespace Metadata
	{
		...
		PCG_API void LogFailToCreateAttributeError(const FPCGMetadataAttributeDesc& Desc, const FPCGContext* InContext = nullptr);

		template <typename T>
		void LogFailToCreateAttributeError(const FText& AttributeName, const FPCGContext* InContext = nullptr)
		{
			LogFailToCreateAttributeError(PCG::Private::MakeAttributeDesc<T>(FName(AttributeName.ToString())), InContext);
		}

		template <typename T>
		void LogFailToCreateAttributeError(FName AttributeName, const FPCGContext* InContext = nullptr)
		{
			LogFailToCreateAttributeError(PCG::Private::MakeAttributeDesc<T>(AttributeName), InContext);
		}
	...
	{
		PCG_API void LogInvalidRotationsReset(const FPCGContext* InContext = nullptr);
		PCG_API void LogInvalidRotationsSkipped(const FPCGContext* InContext = nullptr);
	}
}
