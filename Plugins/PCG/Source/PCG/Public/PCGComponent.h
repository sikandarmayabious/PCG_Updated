...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnPCGLinkClearedExternal, UPCGComponent*, PCGComponent, UObject*, NewOwner);
...
UENUM()
enum class EPCGIgnoreChangeOriginReason : uint8
{
	None = 0,
...
class UPCGComponent : public UActorComponent, public IPCGGraphExecutionSource
{
	...
	friend class FPCGTrackingManager;
...
public:
	...
	/**
	 * Returns the underlying PCG Graph for this component.
	 * Note that this is not the Graph Instance on the PCG Component, but the PCG Graph asset at the top of the instance hierarchy.
	 * If it is needed to set Graph Parameters on the component, use the GraphInstance.
	 */
...
public:
	UE_API bool AreManagedResourcesAccessible() const;
	...
	UE_API void GenerateLocal(EPCGComponentGenerationTrigger RequestedGenerationTrigger, bool bForce, uint32 Grid = PCGHiGenGrid::UninitializedGridSize(), const TArray<FPCGTaskId>& Dependencies = {});

	UE_API FPCGTaskId GenerateLocalGetTaskId(bool bForce);
	UE_API FPCGTaskId GenerateLocalGetTaskId(EPCGComponentGenerationTrigger RequestedGenerationTrigger, bool bForce, uint32 Grid = PCGHiGenGrid::UninitializedGridSize());
	UE_API FPCGTaskId GenerateLocalGetTaskId(EPCGComponentGenerationTrigger RequestedGenerationTrigger, bool bForce, uint32 Grid, const TArray<FPCGTaskId>& Dependencies);
	...
	UE_API AActor* ClearPCGLink(const FPCGMoveResourceParams& Params);

	/**
	 * Converts generated component resources into persistent instance components without moving them away from this component's owner.
	 * The components remain on the same actor, but PCG no longer owns or deletes them during the next generation.
	 * Intended for editor tools that append independent generated strokes with one authoring PCG component.
	 */
	UE_API bool BakeGeneratedComponentsInPlace();

#if WITH_EDITOR
	/** Bakes now, or immediately after an in-flight interactive generation completes. */
	UE_API void RequestBakeGeneratedComponentsInPlace();
#endif
	...
	/** Get the grid that this component is executing on without grid scaling applied. */
	EPCGHiGenGrid GetUnscaledGenerationGrid() const;
	...
	UE_API double GetGenerationRadiusFromGrid(uint32 Grid) const;

	/** Compute the runtime cleanup radius for the given (scaled) grid size. */
	UE_API double GetCleanupRadiusFromGrid(uint32 Grid) const;

	/** Allows executing runtime PCG graphs without partition actors and local PCG components. */
	UE_API bool UseActorComponentlessGeneration() const;
...
public:
	...
	/** Event dispatched when a component has finished to do its Clear PCG Link. */
	UPROPERTY(BlueprintAssignable, meta = (DisplayName = "On PCG Link Cleared"))
	FOnPCGLinkClearedExternal OnPCGLinkClearedExternal;
	...
	*/
	...
	/** Return if this component was already generated when it was loaded */
	bool IsGeneratedOffline() const { return bGeneratedOffline; }

	/** Returns task ids to do internal chaining */
	FPCGTaskId GetGenerationTaskId() const { return CurrentGenerationTask; }
	FPCGTaskId GetCleanupTaskId() const { return CurrentCleanupTask; }
	...
	UE_API bool IsIgnoringAnyChangeOrigins(const TArrayView<const UObject*> InChangeOrigins, const UObject*& OutFirstObjectFound, EPCGIgnoreChangeOriginReason& OutReason) const;
	...
	*/
	...
	UE_API FBox GetTotalBounds() const;
...
protected:
#if WITH_EDITOR
	UE_API void OnPendingBakeGeneratedComponents(UPCGComponent* GeneratedComponent);
	FDelegateHandle PendingBakeGeneratedComponentsHandle;
#endif
...
private:
	...
	UE_API FPCGTaskId GenerateInternal(bool bForce, uint32 Grid, EPCGComponentGenerationTrigger RequestedGenerationTrigger, const TArray<FPCGTaskId>& Dependencies);
	...
	UE_API bool MoveResourcesToNewActor(const FPCGMoveResourceParams& Params, bool bCreateChild);
	...
	UE_API void OnManagedResourceAdded(UPCGManagedResource* InResource);
...
protected:
	UE_API virtual TSharedPtr<ITransactionObjectAnnotation> FactoryTransactionAnnotation(const ETransactionAnnotationCreationMode InCreationMode) const override;
...
private:
	...
	UE_DEPRECATED(5.8, "Moved to ManagedResourceContainer")
	UPROPERTY()
	TArray<TObjectPtr<UPCGManagedResource>> GeneratedResources_DEPRECATED;
	...
	// In order to be more efficient for tracking when a partitioned PCG Component depends on another partitioned PCG component, keep the accumulated
	// bounds of all the local changes.
	UPROPERTY(Transient)
	FBox LocalChangedBounds = FBox(EForceInit::ForceInit);

	mutable PCG::FLock LocalChangedBoundsLock;
#endif // WITH_EDITORONLY_DATA

	// Flag to indicate whether this component was already generated on load.
	bool bGeneratedOffline = false;
	...
	*/
	...
	/** Manual override for the enabling actor componentless generation. */
	UPROPERTY(EditAnywhere, Category = "RuntimeGeneration|Advanced", meta = (EditCondition = "GenerationTrigger == EPCGComponentGenerationTrigger::GenerateAtRuntime", EditConditionHides))
	bool bOverrideUseActorComponentlessGeneration = false;

	/** [EXPERIMENTAL] Avoids creating partition actors and local PCG components. Any functionality which depends on a target actor/component are not supported in this mode. */
	UPROPERTY(EditAnywhere, Category = "RuntimeGeneration|Advanced", meta = (EditCondition = "GenerationTrigger == EPCGComponentGenerationTrigger::GenerateAtRuntime && bOverrideUseActorComponentlessGeneration", EditConditionHides))
	bool bUseActorComponentlessGeneration = false;
...
private:
	/** Generalized storage for the execution source (ex. tool data, runtime gen data, manual overrides, etc.) */
	UPROPERTY()
	FPCGSourceDataContainer SourceDataContainer;

	FPCGComponentExecutionState ExecutionState;

	// Deprecated section
public:
	UE_DEPRECATED(5.8, "Use the version that takes uint32 grid size instead")
	...
	UE_DEPRECATED(5.8, "Use the version that takes uint32 grid size instead")
	UE_API FPCGTaskId GenerateLocalGetTaskId(EPCGComponentGenerationTrigger RequestedGenerationTrigger, bool bForce, EPCGHiGenGrid Grid);
	UE_DEPRECATED(5.8, "Use the version that takes uint32 grid size instead")
	...
	UE_DEPRECATED(5.8, "Use GetGenerationGridSize")
	EPCGHiGenGrid GetGenerationGrid() const { return PCGHiGenGrid::GridSizeToGrid(GetGenerationGridSize()); }
	UE_DEPRECATED(5.8, "Use version that takes uint32 grid size instead")
	double GetGenerationRadiusFromGrid(EPCGHiGenGrid Grid) const { return GetGenerationRadiusFromGrid(PCGHiGenGrid::GridToGridSize(Grid)); }
	UE_DEPRECATED(5.8, "Use version that takes uint32 grid size instead")
	double GetCleanupRadiusFromGrid(EPCGHiGenGrid Grid) const { return GetCleanupRadiusFromGrid(PCGHiGenGrid::GridToGridSize(Grid)); }
};

USTRUCT()
struct UE_DEPRECATED(5.8, "Use FPCGTrackedKeyInstanceData instead") FDynamicTrackedKeyInstanceData
{
	GENERATED_BODY();
...
USTRUCT()
struct FPCGTrackedKeyInstanceData
{
	GENERATED_BODY();
...
protected:
	...
	// UPCGComponent::bGeneratedOffline is not a property so we need to capture/apply it ourselves
	UPROPERTY()
	bool bGeneratedOffline = false;

	// UPCGComponent::DynamicallyTrackedKeysToSettings/StaticallyTrackedKeysToSettings are not properties so we need to capture/apply them ourselves.
	// To avoid having to write custom serialization code the map is converted into an array for serialization.
	UPROPERTY()
	TArray<FPCGTrackedKeyInstanceData> DynamicallyTrackedKeysToSettings;

	UPROPERTY()
	TArray<FPCGTrackedKeyInstanceData> StaticallyTrackedKeysToSettings;
#endif
};
