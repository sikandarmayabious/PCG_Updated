...
struct FPCGMeshSelectorWeightedEntry
{
	...
	/**
	 * Local transform applied only to instances spawned from this mesh entry.
	 * It is composed after the input point transform, so it does not modify any
	 * other weighted mesh or the Static Mesh Spawner's Global Transform.
	 */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (DisplayName = "Mesh Transform Override"))
	FTransform MeshTransformOverride = FTransform::Identity;
