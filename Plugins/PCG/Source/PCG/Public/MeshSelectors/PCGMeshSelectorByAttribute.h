...
public:
	...
	/**
	 * Optional transform metadata generated for each selected mesh entry. When
	 * present, it is applied locally to that entry's instances after point
	 * placement. This keeps it independent of the shared spawner transform.
	 */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = MeshSelector)
	FName PerMeshTransformAttribute = TEXT("MeshTransformOverride");
