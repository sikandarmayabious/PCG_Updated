...
bool UPCGMeshSelectorWeightedByCategory::SelectMeshInstances(
	...
	UPCGBasePointData* OutPointData) const
{
	...
	if (Context.CurrentPointIndex == 0)
	{
		...
		for (const FPCGWeightedByCategoryEntryList& Entry : Entries)
		{
			...
			for (const FPCGMeshSelectorWeightedEntry& WeightedEntry : Entry.WeightedMeshEntries)
			{
				...
				InstancesAndWeights->MeshTransformOverrides.Add(WeightedEntry.MeshTransformOverride);
	...
	while (CurrentPointIndex < InPointData->GetNumPoints())
	{
		...
		const PCGMetadataValueKey ValueKey = Attribute->GetValueKey(InMetadataEntry);
		...
		{
			check(InstancesAndWeights->MeshTransformOverrides.IsValidIndex(RandomPick));
			const FTransform InstanceTransform = InTransform * InstancesAndWeights->MeshTransformOverrides[RandomPick];
			const bool bNeedsReverseCulling = (InstanceTransform.GetDeterminant() < 0);
			FPCGMeshInstanceList& InstanceList = PCGMeshSelectorWeighted::GetInstanceList(InstancesAndWeights->MeshInstances[RandomPick], bUseAttributeMaterialOverrides, MaterialOverrideHelper.GetMaterialOverrides(InMetadataEntry), bNeedsReverseCulling, InPointData);
	...
	if (CurrentPointIndex == InPointData->GetNumPoints())
	{
		if (OutPointData)
		{
