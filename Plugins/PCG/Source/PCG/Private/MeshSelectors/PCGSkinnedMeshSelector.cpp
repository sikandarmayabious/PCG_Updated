	...
	UPCGPointData* OutPointData) const
{
	...
	const FPCGAttributePropertyInputSelector MeshAttributeSelector = MeshAttribute.CopyAndFixLast(InPointData);
	const FName MeshAttributeName = MeshAttributeSelector.GetAttributeName();

	if(!InPointData->Metadata->HasAttribute(MeshAttributeName))
	{
		PCGE_LOG_C(Error, GraphAndLog, &Context, FText::Format(LOCTEXT("MeshAttributeNotInMetadata", "Mesh Attribute '{0}' is not in the metadata."), FText::FromName(MeshAttributeName)));
		return true;
	}

	const FPCGMetadataAttributeBase* MeshAttributeBase = InPointData->Metadata->GetConstAttribute(MeshAttributeName);
	...
	{
		PCGE_LOG_C(Error, GraphAndLog, &Context, FText::Format(LOCTEXT("MeshAttributeInvalidType", "Mesh Attribute '{0}' is not of valid type (must be FString or FSoftObjectPath)."), FText::FromName(MeshAttributeName)));
		return true;
	}
...

	{
		...
		while (CurrentPartitionIndex < Context.AttributeOverridePartition.Num())
		{
			...
			bool bHasInvalidRotations = false;

			auto AddPointsToInstanceList = [&OutMeshInstances, &CurrentPartitionDescriptor, &Context, &MaterialOverrideHelper, &Points, &bHasInvalidRotations, PartitionIndex, InPointData](const TArray<int32>& PointIndices, bool bReverseTransform)
			{
				if (MaterialOverrideHelper.OverridesMaterials())
				{
					for (int32 PointIndex : PointIndices)
					{
						...
						if (!Point.Transform.IsRotationNormalized())
						{
							bHasInvalidRotations = true;
							continue;
						}
						...
						//{
						//	BankItem.BankAsset = Context.PointBankValues[PointIndex];
						//	BankItem.SequenceIndex = Context.PointSequenceIndexValues[PointIndex];
						//}
				...
				else
				{
					...
					InstanceList.InstancePointIndices.Reserve(InstanceOffset + PointIndices.Num());
					...
					for (int32 PointIndex : PointIndices)
					{
						...
						// GW-TODO: 
						//{
							//BankItem.BankAsset = Context.PointProviderValues[PointIndex];
							//BankItem.SequenceIndex = Context.PointAnimationValues[PointIndex];
						//}
