	...
	UPCGBasePointData* OutPointData) const
{
	...
	if (Context.CurrentPointIndex == 0)
	{
		...
		for (const FPCGMeshSelectorWeightedEntry& Entry : MeshEntries)
		{
			...
			Context.WeightedMeshTransformOverrides.Add(Entry.MeshTransformOverride);
	...
	// Assign points to entries
	{
		...
		while (CurrentPointIndex < InPointData->GetNumPoints())
		{
			...
			{
				check(Context.WeightedMeshTransformOverrides.IsValidIndex(RandomPick));
				const FTransform InstanceTransform = InTransform * Context.WeightedMeshTransformOverrides[RandomPick];
