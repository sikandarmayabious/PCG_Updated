...
{
	static FTransform ApplyLocalMeshTransform(const FTransform& PointTransform, const FTransform& MeshTransformOverride)
	{
		// Apply the entry transform in the point's local space. This preserves the
		// point's orientation on the sampled surface while allowing each mesh entry
		// to have its own offset, rotation, and scale.
		FTransform Result = PointTransform;
		Result.SetLocation(PointTransform.TransformPosition(MeshTransformOverride.GetLocation()));
		Result.SetRotation(PointTransform.GetRotation() * MeshTransformOverride.GetRotation());
		Result.SetScale3D(PointTransform.GetScale3D() * MeshTransformOverride.GetScale3D());
		return Result;
	}
	...
	UPCGBasePointData* OutPointData) const
{
	...
	// Draw Spline Surface copies the selected mesh-entry struct onto the point
	// metadata. Its user-defined-struct field is surfaced as MeshTransformOverride,
	// which lets the selector apply a different transform to each chosen mesh.
	const FPCGMetadataAttribute<FTransform>* PerMeshTransformMetadataAttribute = nullptr;
	if (PerMeshTransformAttribute != NAME_None)
	{
		PerMeshTransformMetadataAttribute = InPointData->Metadata->GetConstTypedAttribute<FTransform>(PerMeshTransformAttribute);

		// User Defined Struct properties have generated internal names in older
		// tool assets. Accept that generated suffix as a backwards-compatible path.
		if (!PerMeshTransformMetadataAttribute)
		{
			TArray<FName> AttributeNames;
			TArray<EPCGMetadataTypes> AttributeTypes;
			InPointData->Metadata->GetAttributes(AttributeNames, AttributeTypes);

			for (int32 AttributeIndex = 0; AttributeIndex < AttributeNames.Num(); ++AttributeIndex)
			{
				if (AttributeTypes[AttributeIndex] == EPCGMetadataTypes::Transform
					&& AttributeNames[AttributeIndex].ToString().StartsWith(PerMeshTransformAttribute.ToString()))
				{
					PerMeshTransformMetadataAttribute = InPointData->Metadata->GetConstTypedAttribute<FTransform>(AttributeNames[AttributeIndex]);
					break;
				}
			}
		}
	...
	if (!Context.bPartitionDone)
	{
		...
		static FPCGSoftISMComponentDescriptor DummyDescriptor{};
		...
		// Remove all overrides that target data-level specifically
		// @todo_pcg: warning - there`s an assumption that the override attributes aren't global attributes, which we really need to change.
		for (int SelectorIndex = ValidSelectorOverrides.Num() - 1; SelectorIndex >= 0; --SelectorIndex)
		{
			if(InPointData->GetMetadataDomainIDFromSelector(ValidSelectorOverrides[SelectorIndex]) == PCGMetadataDomainID::Data)
			{
				ValidSelectorOverrides.RemoveAt(SelectorIndex, EAllowShrinking::No);
			}
		}
	...
	int32 CurrentPartitionIndex = Context.CurrentPointIndex; // misnomer but we're reusing another concept from the context
	{
		...
		auto GetInstanceTransform = [&InTransformRange, &InMetadataEntryRange, PerMeshTransformMetadataAttribute](int32 PointIndex)
		{
			const FTransform& PointTransform = InTransformRange[PointIndex];
			return PerMeshTransformMetadataAttribute
				? PCGMeshSelectorAttribute::ApplyLocalMeshTransform(PointTransform, PerMeshTransformMetadataAttribute->GetValueFromItemKey(InMetadataEntryRange[PointIndex]))
				: PointTransform;
		};
		...
		while(CurrentPartitionIndex < Context.AttributeOverridePartition.Num())
		{
			...
			TArray<int32> ReverseInstances;
			{
				...
				{
					const FTransform InstanceTransform = GetInstanceTransform(Partition[ReadIndex]);
					if (InstanceTransform.GetDeterminant() < 0)
					{
			...
			auto AddPointsToInstanceList = [&OutMeshInstances, &CurrentPartitionDescriptor, &MaterialOverrideHelper, &InMetadataEntryRange, &GetInstanceTransform, &bHasInvalidRotation, PartitionIndex, InPointData](const TArray<int32>& PointIndices, bool bReverseTransform)
			{
				if (MaterialOverrideHelper.OverridesMaterials())
				{
					for(int32 PointIndex : PointIndices)
					{
						const FTransform InstanceTransform = GetInstanceTransform(PointIndex);
						...
						const int64& InMetadataEntry = InMetadataEntryRange[PointIndex];
						...
						InstanceList.Instances.Emplace(InstanceTransform);
				...
				else
				{
					...
					InstanceList.Instances.Reserve(InstanceOffset + PointIndices.Num());
					InstanceList.InstancesIndices.Reserve(InstanceOffset + PointIndices.Num());
...

			{
				...
				// @todo_pcg: we should log this only once, not once-per-call, but this requires a change in the context.
				if (bHasInvalidRotation)
				{
					PCGLog::Validation::LogInvalidRotationsSkipped();
				}
			}
