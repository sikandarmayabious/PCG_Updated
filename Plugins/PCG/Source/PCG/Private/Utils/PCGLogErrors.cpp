...
{
	if (const FPCGContext* Context = InContext ? InContext : FPCGContext::GetContextForThread())
	{
		PCGE_LOG_C(Error, GraphAndLog, Context, InMsg);
	}
	else
	{
		UE_LOGF(LogPCG, Error, "[NOCONTEXT] %ls", *InMsg.ToString());
	}
}
...
void PCGLog::LogWarningOnGraph(const FText& InMsg, const FPCGContext* InContext)
{
	if (const FPCGContext* Context = InContext ? InContext : FPCGContext::GetContextForThread())
	{
		PCGE_LOG_C(Warning, GraphAndLog, Context, InMsg);
	}
	else
	{
		UE_LOGF(LogPCG, Warning, "[NOCONTEXT] %ls", *InMsg.ToString());
	}
}
...
namespace PCGLog::InputOutput
{
	namespace Format
	{
		...
		const FTextFormat FileNotFound = LOCTEXT("FileNotFound", "File was not found at given path: '{0}'");
		const FTextFormat InvalidFileType = LOCTEXT("InvalidFileType", "File was of the wrong type: '{0}' Expected type: {1}");
		const FTextFormat DirectoryNotFound = LOCTEXT("DirectoryNotFound", "Directory was not found at given path: '{0}'");
	}
	...
	void LogTypedDataNotFoundWarning(EPCGDataType DataType, const FName PinLabel, const FPCGContext* InContext)
	{
		...
		LogWarningOnGraph(FText::Format(Format::TypedInputNotFound, TypeText, FText::FromName(PinLabel)), InContext);
	}
	...
	void LogFileNotFound(const FString& FilePath, const FPCGContext* InContext)
	{
		LogErrorOnGraph(FText::Format(Format::FileNotFound, FText::FromString(FilePath)), InContext);
	}

	void LogInvalidFileType(const FString& FileType, const FString& ExpectedType, const FPCGContext* InContext)
	{
		LogErrorOnGraph(FText::Format(Format::InvalidFileType, FText::FromString(FileType), FText::FromString(ExpectedType)), InContext);
	}

	void LogDirectoryNotFound(const FString& DirectoryPath, const FPCGContext* InContext)
	{
		LogErrorOnGraph(FText::Format(Format::DirectoryNotFound, FText::FromString(DirectoryPath)), InContext);
	}
}
...
namespace PCGLog::Metadata
{
	...
	void LogFailToCreateAttributeError(const FPCGMetadataAttributeDesc& AttributeDesc, const FPCGContext* InContext)
	{
		LogErrorOnGraph(FText::Format(Format::CreateAttributeFailure, FText::FromName(AttributeDesc.Name), AttributeDesc.GetTypeText()), InContext);
	}
...
namespace PCGLog::Validation
{
	namespace Format
	{
		const FText InvalidRotationReset = LOCTEXT("InvalidRotationsReset", "Some elements have an invalid rotation and had their rotation reset. Make sure you are not setting individual components of the rotation quaternion (like `$Rotation.X`) without normalizing the quaternion afterwards.");
		const FText InvalidRotationSkip = LOCTEXT("InvalidRotationsSkip", "Some elements have an invalid rotation and were skipped. Make sure you are not setting individual components of the rotation quaternion (like `$Rotation.X`) without normalizing the quaternion afterwards.");
	}

	void LogInvalidRotationsReset(const FPCGContext* InContext)
	{
		LogErrorOnGraph(Format::InvalidRotationReset, InContext);
	}

	void LogInvalidRotationsSkipped(const FPCGContext* InContext)
	{
		LogErrorOnGraph(Format::InvalidRotationSkip, InContext);
	}
}
