...
static TAutoConsoleVariable<bool> CVarPCGValidatePinAndTypes(
	TEXT("pcg.debug.ValidateDataPinsAndTypes"),
	true,
	TEXT("Controls whether we validate that output data from nodes are assigned to valid pins and have proper types for their pins."));
...
#define PCG_ELEMENT_EXECUTION_BREAKPOINT() \
	if (Context && Context->GetInputSettingsInterface() && Context->GetInputSettingsInterface()->bBreakDebugger && Context->IsExecutingGraphInspected()) \
	{ \
		UE_DEBUG_BREAK(); \
	}
#else
...
bool IPCGElement::Execute(FPCGContext* Context) const
{
	...
	FPCGContextScope ContextScope(Context);
...
void IPCGElement::PreExecutePrimaryLoopElement(FPCGContext* Context, const UPCGSettings* Settings) const
{
	...
	// No need to split data if we're not using the cache.
...
{
	if (Context->bConvertInputsIfNeededDone)
	{
		return true;
	}
	...
	{
		Context->bConvertInputsIfNeededDone = true;
		return true;
	}

	if (!SupportsGPUResidentData(Context))
	{
		// Multi-stage GPU readback. The stages are sequential rather than parallel because the graph executor does not support mixing bIsPaused polling with
		// DynamicDependencies in the same task. The texture readback must be scheduled on the game thread because it can affect the texture source.

		// Stage 1: proxy readbacks (inline poll).
		{
			bool bHasPendingProxyReadbacks = false;
			...
			for (FPCGTaggedData& TaggedData : Context->InputData.TaggedData)
			{
				if (const UPCGProxyForGPUData* DataWithGPUSupport = Cast<UPCGProxyForGPUData>(TaggedData.Data))
				{
					...
					{
						bHasPendingProxyReadbacks = true;
					}
				}
			}

			if (bHasPendingProxyReadbacks)
			{
				Context->bIsPaused = true;

				// Unpause on the next game-thread tick so the scheduler re-enters this element and polls again.
		...
		// Stage 2: texture readbacks (GT-only generic task, sleep via DynamicDependencies).
		{
			bool bHasPendingTextureReadbacks = false;
			for (const FPCGTaggedData& TaggedData : Context->InputData.TaggedData)
			{
				if (const UPCGTexture2DSingleBaseData* TextureData = Cast<UPCGTexture2DSingleBaseData>(TaggedData.Data))
				{
					if (!TextureData->IsReadbackComplete())
					{
						bHasPendingTextureReadbacks = true;
						break;
					}
				}
			}

			if (bHasPendingTextureReadbacks)
			{
				auto DoTextureReadbacks = [ContextHandle = Context->GetOrCreateHandle()](FPCGContext*)
				{
					TSharedPtr<FPCGContextHandle> SharedHandle = ContextHandle.Pin();
					FPCGContext* ContextPtr = SharedHandle ? SharedHandle->GetContext() : nullptr;
					if (!ContextPtr)
					{
						return true;
					}
					...
					for (const FPCGTaggedData& TaggedData : ContextPtr->InputData.TaggedData)
					{
						if (const UPCGTexture2DSingleBaseData* TextureData = Cast<UPCGTexture2DSingleBaseData>(TaggedData.Data))
						{
							if (!TextureData->IsReadbackComplete())
							{
#if WITH_EDITOR
								if (ContextPtr->Node && ContextPtr->GetStack() && ContextPtr->ExecutionSource.IsValid())
								{
									ContextPtr->ExecutionSource->GetExecutionState().GetInspection().NotifyGPUToCPUReadback(ContextPtr->Node, ContextPtr->GetStack());
								}
#endif

								if (!TextureData->RequestCPUReadback())
								{
									bAllComplete = false;
								}
							}
				...
				FPCGScheduleGenericParams Params(DoTextureReadbacks, Context->ExecutionSource.Get());
				Params.bCanExecuteOnlyOnMainThread = true;

				const FPCGTaskId TextureReadbackTaskId = Context->ScheduleGeneric(Params);
				if (ensure(TextureReadbackTaskId != InvalidPCGTaskId))
				{
					Context->DynamicDependencies.Add(TextureReadbackTaskId);
					return false;
				}
			}
		}
	...
	if (EnumHasAnyFlags(GetDataOverridePhase(), EPCGDataOverridePhase::PrepareData))
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(IPCGElement::ConvertInputsIfNeeded::ApplyDataOverrides);

		const bool bHasNodeLevelOverrides = PCG::DataOverride::HasNodeLevelDataOverrides(Context);
		bool bHasAnyOverrides = bHasNodeLevelOverrides;

		// Need to duplicate the data in pre-application phase to avoid stomping data shared by other elements.
		// @todo_pcg: Consider data stealing if possible.
		for (FPCGTaggedData& TaggedData : Context->InputData.TaggedData)
		{
			// If we have node overrides, all data will be copied. If not, check for pin level overrides.
			if (TaggedData.Data && (bHasNodeLevelOverrides || PCG::DataOverride::HasPinLevelDataOverrides(Context, TaggedData)))
			{
				TaggedData.Data = TaggedData.Data->DuplicateData(Context);
				bHasAnyOverrides = true;
			}
		}

		if (bHasAnyOverrides)
		{
			PCG::DataOverride::ApplyDataOverrides(Context, Context->InputData.TaggedData);

#if WITH_EDITOR
			// Notify inspection that input data overrides were applied (pre-execute).
			// @todo_pcg: Track orphaned overrides
			if (Context->Node && Context->GetStack() && Context->ExecutionSource.IsValid())
			{
				Context->ExecutionSource->GetExecutionState().GetInspection().NotifyDataOverridesApplied(Context->Node, Context->GetStack());
			}
#endif
		}
	}
...
void IPCGElement::PostExecute(FPCGContext* Context) const
{
	...
	// Data overrides must be applied before the Crc
	if (SettingsInterface
		&& SettingsInterface->bEnabled // Ignore overrides if node is disabled.
		&& !Context->OutputData.TaggedData.IsEmpty()
		&& EnumHasAnyFlags(GetDataOverridePhase(), EPCGDataOverridePhase::PostExecute)
		&& PCG::DataOverride::HasAnyDataOverrides(Context, Context->OutputData.TaggedData))
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(IPCGElement::PostExecute::ApplyDataOverrides);
		PCG::DataOverride::ApplyDataOverrides(Context, Context->OutputData.TaggedData);

#if WITH_EDITOR
		// Notify inspection that output data overrides were applied (post-execute).
		// @todo_pcg: Track orphaned overrides
		if (Context->Node && Context->GetStack() && Context->ExecutionSource.IsValid())
		{
			Context->ExecutionSource->GetExecutionState().GetInspection().NotifyDataOverridesApplied(Context->Node, Context->GetStack());
...
void IPCGElement::PostExecutePrimaryLoopElement(FPCGContext* Context, const UPCGSettings* Settings) const
{
	...
	if (!Settings || !Context->IsCacheEnabled())
	{
		return;
	}
...
bool IPCGElement::DebugDisplay(FPCGContext* Context) const
{
	...
	// If graph is being inspected, only display Debug if the execution source is being inspected, or in the HiGen case also display if
	...
	UPCGGraph* Graph = ExecutionSource ? ExecutionSource->GetExecutionState().GetGraph() : nullptr;
	if (Graph && Graph->IsInspecting() && !ExecutionSource->GetExecutionState().GetInspection().IsInspecting() && Graph->DebugFlagAppliesToIndividualComponents())
	{
		...
		if (UPCGSubsystem* Subsystem = Cast<UPCGSubsystem>(ExecutionSource->GetExecutionState().GetSubsystem()))
		{
			const uint32 ThisGenerationGridSize = ExecutionSource->GetExecutionState().GetGenerationGridSize();

			bool bFoundInspectedChild = false;

			// @todo_pcg: support execution source
			if(UPCGComponent* OriginalComponent = Cast<UPCGComponent>(ExecutionSource))
			{
				Subsystem->ForAllOverlappingComponentsInHierarchy(OriginalComponent, [ThisGenerationGridSize, &bFoundInspectedChild](UPCGComponent* InLocalComponent)
				{
					if (InLocalComponent->GetExecutionState().GetGenerationGridSize() < ThisGenerationGridSize && InLocalComponent->GetExecutionState().GetInspection().IsInspecting())
					{
						bFoundInspectedChild = true;
					}
				});
			}

			// If no inspected child component then don't display debug.
			if (!bFoundInspectedChild)
			{
				return true;
			}
		}
	...
	for (const FPCGTaggedData& Output : Context->OutputData.TaggedData)
	{
		...
		else if (const UPCGTexture2DSingleBaseData* TextureData = Cast<UPCGTexture2DSingleBaseData>(Output.Data))
		{
			if (!TextureData->IsReadbackComplete())
			{
				if (Context->Node && Context->GetStack() && Context->ExecutionSource.IsValid())
				...
				// Schedule a game-thread-only task to trigger the readback and poll until complete. The readback must run on the game thread because touching the texture source on a worker thread crashes.
				auto DoReadback = [WeakTextureData = MakeWeakObjectPtr(TextureData)](FPCGContext*)
				{
					const UPCGTexture2DSingleBaseData* TexData = WeakTextureData.Get();
					return !TexData || TexData->RequestCPUReadback();
				};

				FPCGScheduleGenericParams Params(DoReadback, Context->ExecutionSource.Get());
				Params.bCanExecuteOnlyOnMainThread = true;

				FPCGTaskId ReadbackTaskId = Context->ScheduleGeneric(Params);
				if (ReadbackTaskId != InvalidPCGTaskId)
				{
					Context->DynamicDependencies.Add(ReadbackTaskId);
				}
	...
	for (const FPCGTaggedData& Data : DataToDebug)
	{
		...
		{
			DataVis->ExecuteDebugDisplay(Context, SettingsInterface, Data.Data, Context->GetDebugDisplayActor());
		}
	}
...
#if WITH_EDITOR
		const bool bNeedsPinsAndTypesValidation = CVarPCGValidatePinAndTypes.GetValueOnAnyThread();
		const bool bNeedsMetadataValidation =  CVarPCGValidatePointMetadata.GetValueOnAnyThread();

		if (SettingsInterface->bEnabled && (bNeedsPinsAndTypesValidation || bNeedsMetadataValidation))
		{
			if (bNeedsPinsAndTypesValidation)
			{
				// remove null outputs
			...
			for (FPCGTaggedData& TaggedData : Context->OutputData.TaggedData)
			{
				...
				if (bNeedsMetadataValidation)
				{
					...
					if (const UPCGMetadata* Metadata = TaggedData.Data.Get() ? TaggedData.Data->ConstMetadata() : nullptr)
					{
						const bool bMetadataHasParent = (Metadata->GetParent() != nullptr);

						TArray<FPCGMetadataDomainID> DomainIDs = TaggedData.Data->GetAllSupportedMetadataDomainIDs();
						for (FPCGMetadataDomainID DomainID : DomainIDs)
						{
							if (const FPCGMetadataDomain* Domain = Metadata->GetConstMetadataDomain(DomainID))
							{
								const bool bDomainHasParent = (Domain->GetParent() != nullptr);

								if (bDomainHasParent && !bMetadataHasParent)
								{
									PCGE_LOG(Error, GraphAndLog, FText::Format(LOCTEXT("InvalidDomainParenting", "Invalid parenting on the domain '{0}'."), FText::FromName(DomainID.DebugName)));
								}
							}
						}
					}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	else
	{
		...
		UE_LOGF(LogPCG, Warning, "This node '%ls' implements a deprecated version of Initialize. Please implement version with FPCGInitializeElementParams parameter.", *SettingsName);
	}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (!OutCrc.IsValid() || CopyCrc == OutCrc)
	{
		...
		const FPCGSourceDataContainer* DataContainer = InParams.ExecutionSource ? InParams.ExecutionSource->GetExecutionState().GetSourceDataContainer() : nullptr;
		if (DataContainer && !DataContainer->IsEmpty())
		{
			// @todo_pcg: This ties a non-empty data container to every element's Crc. If the data within the container is changed,
			// every element's Crc will be invalidated. To be addressed in a secondary pass.
			Crc.Combine(DataContainer->GetDirtyGeneration());
		}
	...
	else
	{
		...
		UE_LOGF(LogPCG, Warning, "This node '%ls' implements a deprecated version of GetDependenciesCrc. Please implement version with FPCGGetDependenciesCrcParams parameter.", *SettingsName);
	}
}

bool IPCGElement::BuildCacheParams(IPCGGraphCache* Cache, const UPCGNode* Node, const FPCGDataCollection& Input, IPCGGraphExecutionSource* ExecutionSource, FPCGGetFromCacheParams& OutCacheParams) const
{
	...
	OutCacheParams = { .Node = Node, .Element = this, .ExecutionSource = ExecutionSource };

	if (Settings && bCacheable)
	{
		GetDependenciesCrc(FPCGGetDependenciesCrcParams(&Input, Settings, ExecutionSource), OutCacheParams.Crc);
		return true;
	}
	else
	{
...
EPCGCachingStatus IPCGElement::RetrieveResultsFromCache(IPCGGraphCache* Cache, const FPCGGetFromCacheParams& InCacheParams, FPCGDataCollection& Output) const
{
	check(Cache && Cache->IsEnabled() && InCacheParams.Crc.IsValid());

	if (Cache->GetFromCache(InCacheParams, Output))
	{
