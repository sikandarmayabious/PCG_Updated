...
namespace PCGMetadataHelpers
{
	...
	bool CopyAttributes(UPCGData* TargetData, const UPCGData* SourceData, const TArray<TTuple<FPCGAttributePropertyInputSelector, FPCGAttributePropertyOutputSelector, EPCGMetadataTypes>>& AttributeSelectorsWithOutputType, bool bSameOrigin, FPCGContext* OptionalContext)
	{
		...
		for (const auto& [InputSource, OutputTarget, RequestedOutputType] : AttributeSelectorsWithOutputType)
		{
			...
			else // Create a new attribute of the accessed field's type manually
			{
				...
				FPCGMetadataAttributeDesc OutputDesc{};
				if (bOutputTypeCast)
				{
					OutputDesc.ValueType = RequestedOutputType;
				}
				else
				{
					OutputDesc = InputAccessor->GetUnderlyingDesc();
				}
				
				if (bOutputTypeCast && InputAccessor->GetUnderlyingDesc().IsSameType(OutputDesc) && bSameOrigin && LocalSourceAttribute == LocalDestinationAttribute)
				{
				...
				if (bOutputTypeCast && !PCG::Private::IsBroadcastableOrConstructible(InputAccessor->GetUnderlyingDesc(), OutputDesc))
				{
					PCGLog::LogWarningOnGraph(FText::Format(LOCTEXT("CastInvalid", "Cannot convert InputAttribute '{0}' of type {1} into {2}"), InputSource.GetDisplayText(), InputAccessor->GetUnderlyingDesc().GetTypeText(), OutputDesc.GetTypeText()), OptionalContext);
					continue;
				}
				...
				bool bShouldCopyDefaultValue = false;
				if (!bOutputHasAnyExtra && bTargetIsAttribute)
				{
					FPCGMetadataDomain* TargetMetadataDomain = TargetMetadata->GetMetadataDomainFromSelector(OutputTarget);
					if (!TargetMetadataDomain)
					{
						PCGLog::Metadata::LogInvalidMetadataDomain(OutputTarget, OptionalContext);
						continue;
					...
					if (TargetMetadataDomain->HasAttribute(OutputDesc.Name))
					{
						TargetMetadataDomain->DeleteAttribute(OutputDesc.Name);
					}
					
					if (!TargetMetadataDomain->CreateAttribute(OutputDesc))
					{
						PCGLog::Metadata::LogFailToCreateAttributeError(OutputDesc, OptionalContext);
						continue;
					}

					bShouldCopyDefaultValue = true;
				}
				...
				if (!bOutputTypeCast && !PCG::Private::IsBroadcastableOrConstructible(OutputDesc, OutputAccessor->GetUnderlyingDesc()))
				{
					PCGLog::LogErrorOnGraph(FText::Format(LOCTEXT("CannotConvertTypes", "Cannot convert input type {0} into output type {1}"), OutputDesc.GetTypeText(), OutputAccessor->GetUnderlyingDesc().GetTypeText()), OptionalContext);
					continue;
				}
				...
				// Do a simple copy of the default value, if it is possible and needed. It is OK to fail if the input accessor is a property for example.
				if (bShouldCopyDefaultValue)
				{
					FPCGAttributeAccessorKeysEntries DefaultEntryKeys{PCGInvalidEntryKey};
					Params.InKeys = &DefaultEntryKeys;
					Params.OutKeys = &DefaultEntryKeys;
					Params.Flags = EPCGAttributeAccessorFlags::AllowBroadcastAndConstructible | EPCGAttributeAccessorFlags::AllowSetDefaultValue;

					PCGMetadataElementCommon::CopyFromAccessorToAccessor(Params);
				}

				// Then do the real copy
				Params.InKeys = InputKeys.Get();
				Params.OutKeys = OutputKeys.Get();
				Params.Flags = EPCGAttributeAccessorFlags::AllowBroadcastAndConstructible;
