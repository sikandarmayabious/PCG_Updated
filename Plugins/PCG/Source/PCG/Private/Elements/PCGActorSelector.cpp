...
namespace PCGActorSelector
{
	...
	TFunction<bool(AActor*)> GetFilteringFunction(const FPCGActorSelectorSettings& InSettings, const TFunction<bool(const AActor*)>& BoundsCheck, const TFunction<bool(const AActor*)>& SelfIgnoreCheck, TArray<AActor*>& InFoundActors)
	{
		...
		case EPCGActorSelection::ByName:
			UE_LOGF(LogPCG, Error, "PCGActorSelector::GetFilteringFunction: Unsupported value for EPCGActorSelection - selection by name is no longer supported.");
			break;
	...
	TArray<AActor*> FindActors(const FPCGActorSelectorSettings& Settings, const IPCGGraphExecutionSource* InExecutionSource, const TFunction<bool(const AActor*)>& BoundsCheck, const TFunction<bool(const AActor*)>& SelfIgnoreCheck, TArrayView<AActor*> InputActors)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(PCGActorSelector::FindActor);

		UWorld* World = InExecutionSource ? InExecutionSource->GetExecutionState().GetWorld() : nullptr;
		AActor* Self = InExecutionSource ? InExecutionSource->GetExecutionState().GetTypedTarget<AActor>() : nullptr;
		...
		// And after filtering, check for adding isolated actors
		auto UpdateFoundActorsWithIsolatedIfNeeded = [IncludeIsolatedActors = Settings.IncludeIsolatedActors](TArray<AActor*>& InOutFoundActors)
		{
			if (IncludeIsolatedActors != EPCGIsolatedActorIncludeFlag::Ignore)
			{
				int ActorIndex = 0;
				while (ActorIndex < InOutFoundActors.Num())
				{
					TArray<APCGIsolatedActor*> IsolatedActors = UPCGActorHelpers::GetIsolatedActorsOriginatingFrom(InOutFoundActors[ActorIndex]);

					if (IncludeIsolatedActors == EPCGIsolatedActorIncludeFlag::Include)
					{
						++ActorIndex;

						if (IsolatedActors.Num() > 0)
						{
							AActor** IsolatedActorsPtr = reinterpret_cast<AActor**>(IsolatedActors.GetData());
							InOutFoundActors.Insert(IsolatedActorsPtr, IsolatedActors.Num(), ActorIndex);
							ActorIndex += IsolatedActors.Num();
						}
					}
					else if (IncludeIsolatedActors == EPCGIsolatedActorIncludeFlag::Only)
					{
						if (IsolatedActors.IsEmpty())
						{
							InOutFoundActors.RemoveAt(ActorIndex);
						}
						else
						{
							InOutFoundActors[ActorIndex++] = IsolatedActors[0];
							if (IsolatedActors.Num() > 1)
							{
								AActor** IsolatedActorsPtr = reinterpret_cast<AActor**>(IsolatedActors.GetData());
								InOutFoundActors.Insert(IsolatedActorsPtr + 1, IsolatedActors.Num() - 1, ActorIndex);
								ActorIndex += IsolatedActors.Num() - 1;
							}
						}
					}
				}
		...
		if (Settings.ActorFilter == EPCGActorFilter::AllWorldActors)
		{
			...
			UpdateFoundActorsWithIsolatedIfNeeded(FoundActors);
		...
		{
			IPCGGraphExecutionSource* OriginalSource = InExecutionSource ? InExecutionSource->GetExecutionState().GetOriginalSource() : nullptr;
			AActor* OriginalActor = OriginalSource ? OriginalSource->GetExecutionState().GetTypedTarget<AActor>() : nullptr;
			if (OriginalActor)
			{
	...
	TArray<AActor*> FindActors(const FPCGActorSelectorSettings* ActorSettings, const FPCGComponentSelectorSettings* ComponentSettings, const IPCGGraphExecutionSource* InExecutionSource, const TFunction<bool(const AActor*)>& BoundsCheck, const TFunction<bool(const AActor*)>& SelfIgnoreCheck, TArrayView<AActor*> InputActors)
	{
		TArray<AActor*> FoundActors;
		if (ActorSettings)
		{
			FoundActors = FindActors(*ActorSettings, InExecutionSource, BoundsCheck, SelfIgnoreCheck, InputActors);
		}
		else
		{
	...
	AActor* FindActor(const FPCGActorSelectorSettings& InSettings, const IPCGGraphExecutionSource* InExecutionSource, const TFunction<bool(const AActor*)>& BoundsCheck, const TFunction<bool(const AActor*)>& SelfIgnoreCheck, TArrayView<AActor*> InputActors)
	{
		...
		TArray<AActor*> Actors = FindActors(Settings, InExecutionSource, BoundsCheck, SelfIgnoreCheck, InputActors);
...
FPCGSelectionKey FPCGSelectionKey::CreateFromObjectPtr(const UObject* InObject)
{
	FPCGSelectionKey Key = FPCGSelectionKey::CreateFromPath(FSoftObjectPath(InObject));
	Key.CachedObject = InObject;
...
{
	// This signature of IsMatching does not support custom keys
	if (CustomKey.IsValid())
	{
		return false;
	}

	const FPCGSelectionKey SelectionKey = FPCGSelectionKey::CreateFromPath(InObjectPath);
	SelectionKey.CachedObject = InObject;
	return IsMatching(SelectionKey, InComponent);
}

bool FPCGSelectionKey::IsMatching(const FPCGSelectionKey& InSelectionKey, const IPCGGraphExecutionSource* InExecutionSource) const
{
	const UObject* SelectionObject = InSelectionKey.GetObjectFromPath();
	if (!SelectionObject)
	{
		return false;
	}
	...
	if (ActorFilter != EPCGActorFilter::AllWorldActors)
	{
		const AActor* InActor = Cast<const AActor>(SelectionObject);
		...
		TArray<AActor*> AllActors = PCGActorSelector::FindActors(SelectorSettings, InExecutionSource, [](const AActor*) { return true; }, [](const AActor*) { return true; });
	...
	case EPCGActorSelection::ByClass:
		return SelectionObject && SelectionObject->GetClass()->IsChildOf(SelectionClass);
	case EPCGActorSelection::ByPath:
		return ObjectPath == InSelectionKey.ObjectPath;
	default:
		return false;
	}
}
...
bool FPCGSelectionKey::IsMatching(const UObject* InObject, const FSoftObjectPath& InObjectPath, const TSet<FName>& InRemovedTags, const TSet<UPCGComponent*>& InComponents, TSet<UPCGComponent*>* OptionalMatchedComponents) const
{
	...
	FPCGSelectionKey SelectionKey = FPCGSelectionKey::CreateFromPath(InObjectPath);
	SelectionKey.CachedObject = InObject;

PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return IsMatching(SelectionKey, InRemovedTags, InComponents, OptionalMatchedComponents);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
}

// deprecated
bool FPCGSelectionKey::IsMatching(const FPCGSelectionKey& InSelectionKey, const TSet<FName>& InRemovedTags, const TSet<UPCGComponent*>& InComponents, TSet<UPCGComponent*>* OptionalMatchedComponents) const
{
	TSet<IPCGGraphExecutionSource*> OptionalMatchedExecutionSources;
	TSet<IPCGGraphExecutionSource*> ExecutionSources;
	ExecutionSources.Reserve(InComponents.Num());
	Algo::Transform(InComponents, ExecutionSources, [](UPCGComponent* InComponent) { return InComponent; });

	bool bReturn = IsMatching(InSelectionKey, InRemovedTags, ExecutionSources, OptionalMatchedComponents ? &OptionalMatchedExecutionSources : nullptr);
	if (OptionalMatchedComponents)
	{
		OptionalMatchedComponents->Reserve(OptionalMatchedExecutionSources.Num());
		Algo::TransformIf(OptionalMatchedExecutionSources, *OptionalMatchedComponents,
...
bool FPCGSelectionKey::IsMatching(const FPCGSelectionKey& InSelectionKey, const TSet<FName>& InRemovedTags, const TSet<IPCGGraphExecutionSource*>& InExecutionSources, TSet<IPCGGraphExecutionSource*>* OptionalMatchedExecutionSources) const
{
	const UObject* SelectionObject = InSelectionKey.GetObjectFromPath();
	...
	if (CustomKey.IsValid() != InSelectionKey.CustomKey.IsValid())
	{
		return false;
	}
	else if (CustomKey.IsValid())
	{
		if (!CustomKey.Get().IsMatching(InSelectionKey.CustomKey.Get()))
		{
			return false;
		}
	}

	// If both keys (this & InSelectionKey) have optional bounds, then check that there is an overlap between any pair.
	if (!OptionalBounds.IsEmpty() && !InSelectionKey.OptionalBounds.IsEmpty())
	{
		bool bHasOverlap = false;
		for (const FBox& Bounds : OptionalBounds)
		{
			if (!Bounds.IsValid)
			{
				continue;
			...
			for (const FBox& OtherBounds : InSelectionKey.OptionalBounds)
			{
	...
	if (ActorFilter != EPCGActorFilter::AllWorldActors)
	{
		FPCGGetExecutionSourcesFromSelectionKeyParams Params(InSelectionKey, ActorFilter);
		TArray<IPCGGraphExecutionSource*> FoundExecutionSources = FPCGModule::GetPCGModuleChecked().GetConstChangeTrackingRegistry().GetExecutionSourcesFromSelectionKey(Params);
		
		bool bFoundMatch = false;

		for (IPCGGraphExecutionSource* FoundExecutionSource : FoundExecutionSources)
		{
			if (InExecutionSources.Contains(FoundExecutionSource))
			{
				bFoundMatch = true;
				if (OptionalMatchedExecutionSources)
				{
					OptionalMatchedExecutionSources->Add(FoundExecutionSource);
				}
				else
				{
					break;
	...
	case EPCGActorSelection::ByClass:
		bIsMatched = SelectionObject->IsA(SelectionClass);
		break;
	case EPCGActorSelection::ByPath:
		bIsMatched = InSelectionKey.ObjectPath == ObjectPath;
		break;
	default:
		bIsMatched = false;
		break;
	...
	if (bIsMatched && OptionalMatchedExecutionSources)
	{
		OptionalMatchedExecutionSources->Append(InExecutionSources);
	}
...
bool FPCGSelectionKey::operator==(const FPCGSelectionKey& InOther) const
{
	...
	if (CustomKey.IsValid() != InOther.CustomKey.IsValid())
	{
		return false;
	}
	else if (CustomKey.IsValid())
	{
		if (!CustomKey.Get().Equals(InOther.CustomKey.Get()))
		{
			return false;
		}
	}

	if (OptionalBounds != InOther.OptionalBounds)
	{
		return false;
	}
...
const UObject* FPCGSelectionKey::GetObjectFromPath() const
{
	if (CachedObject)
	{
		return CachedObject;
	...
	{
		CachedObject = ObjectPath.ResolveObject();
	}
...
uint32 GetTypeHash(const FPCGCustomSelectionKey& In)
{
	return In.GetTypeHashInternal();
}
...
uint32 GetTypeHash(const FPCGSelectionKey& In)
{
	...
	if (In.CustomKey.IsValid())
	{
		HashResult = HashCombine(HashResult, GetTypeHash(In.CustomKey.Get()));
	}

	HashResult = HashCombine(HashResult, GetTypeHash(In.OptionalBounds));
