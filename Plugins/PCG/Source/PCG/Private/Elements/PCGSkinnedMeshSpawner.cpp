...
#if WITH_EDITOR
void UPCGSkinnedMeshSpawnerSettings::CreateKernels(FPCGGPUCompilationContext& InOutContext, UObject* InObjectOuter, const UPCGNode* InNode, TArray<UPCGComputeKernel*>& OutKernels, TArray<FPCGKernelEdge>& OutEdges) const
{
...
bool FPCGSkinnedMeshSpawnerElement::PrepareDataInternal(FPCGContext* InContext) const
{
	...
	if (!Context->bReuseCheckDone)
	{
		...
		if (Context->DependenciesCrc.IsValid() && ManagedResourcesContainerHelper.IsValid())
		{
			TArray<UPCGManagedISKMComponent*> MISKMCs;
			ManagedResourcesContainerHelper.ForEachManagedResource([&MISKMCs, &Context, Settings](UPCGManagedResource* InResource)
			{
...
#if WITH_EDITOR
	FScopedTransaction Transaction(LOCTEXT("AddISKM", "Adding ISKM/instances to an ISKM from PCG"), Context->ExecutionSource.Get() && Context->ExecutionSource->GetExecutionState().UseTransactions());
#endif
...
void FPCGSkinnedMeshSpawnerElement::SpawnSkinnedMeshInstances(FPCGSkinnedMeshSpawnerContext* Context, const FPCGSkinnedMeshInstanceList& InstanceList, AActor* TargetActor, const FPCGSkinnedMeshPackedCustomData* InPackedCustomData) const
{
	...
	UPCGManagedISKMComponent* MISKMC = UPCGActorHelpers::GetOrCreateManagedABMC(TargetActor, Context->ExecutionSource.Get(), Params, Context);
