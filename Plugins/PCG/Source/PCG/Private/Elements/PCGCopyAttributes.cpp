...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGCopyAttributes)
...
bool FPCGCopyAttributesElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	const bool bCopyAllAttributes = Settings->bCopyAllAttributes;
	const bool bCopyAllDomains = bCopyAllAttributes && Settings->bCopyAllDomains;
	...
	for (int32 i = 0; i < NumIterations; ++i)
	{
		...
		auto DoCopy = [this, TargetData, &Sources, Context, Settings, &bSuccess, &OutputData, bCopyAllAttributes, bCopyAllDomains](int32 SourceIndex)
		{
			...
			// Early out when trying to copy all attributes from self, since nothing will happen (Only when we do not have a mapping or copying all domains)
			if (bIsSameData && ((bCopyAllAttributes && Settings->MetadataDomainsMapping.IsEmpty()) || bCopyAllDomains))
			{
			...
			if (bIsSameData && Settings->InputSource == Settings->OutputTarget && !bCopyAllAttributes)
			{
