...
protected:
	...
	/**
	 * Persisted accepted strokes for a Spline Surface tool data instance. They
	 * are emitted together in one PCG execution, so the Static Mesh Spawner owns
	 * one normal managed output instead of baking one ISM per drawn surface.
	 */
	UPROPERTY(VisibleAnywhere, Category = "Working Data")
	TArray<TSoftObjectPtr<USplineComponent>> AcceptedSplineComponents;
...
#if WITH_EDITORONLY_DATA
	/** Previous accepted spline restored as the active data pointer if a newly appended stroke is canceled. */
	UPROPERTY(Transient)
	TSoftObjectPtr<USplineComponent> PreviousAcceptedSplineComponent;
#endif // WITH_EDITORONLY_DATA
};
