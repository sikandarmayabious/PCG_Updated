...
{
	check(InPCGGraph && WeakPCGGraph.IsExplicitlyNull());
	WeakPCGGraph = InPCGGraph;

	InPCGGraph->OnGraphParametersChangedDelegate.AddUObject(this, &UPCGEditorGraph::OnGraphUserParametersChanged);
	InPCGGraph->OnGraphChangedDelegate.AddUObject(this, &UPCGEditorGraph::OnGraphChanged);
...
void UPCGEditorGraph::ReconstructGraph()
{
	...
	auto ConstructNode = [this, &NodeLookup]<typename NodeType>(UPCGNode* PCGNode, TSubclassOf<NodeType> NodeClass = NodeType::StaticClass())
	{
		...
		if (!PCGNode->IsHidden())
		{
			...
			// Need to reset it there because Finalize will override the guid.
			GraphNode->InitNodeGuid();
...
void UPCGEditorGraph::BeginDestroy()
{
	...
	if (UPCGGraph* PCGGraph = GetPCGGraph())
	{
		PCGGraph->OnGraphChangedDelegate.RemoveAll(this);
		PCGGraph->OnGraphParametersChangedDelegate.RemoveAll(this);
	}
}

void UPCGEditorGraph::SaveState()
{
	ReplicateExtraNodes();
}
...
void UPCGEditorGraph::UpdateVisualizations(IPCGGraphExecutionSource* PCGSourceBeingInspected, const FPCGStack* PCGStackBeingInspected)
{
	for (UEdGraphNode* EditorNode : Nodes)
	{
		if (UPCGEditorGraphNodeBase* PCGEditorNode = Cast<UPCGEditorGraphNodeBase>(EditorNode))
		{
			...
			ChangeType |= PCGEditorNode->UpdateOverrideVisualization(PCGSourceBeingInspected, PCGStackBeingInspected);
...
FText UPCGEditorGraph::GetDisplayName() const
{
	FGraphDisplayInfo DisplayInfo;
	GetSchema()->GetGraphDisplayInformation(*this, DisplayInfo);
...
UPCGEditorGraphNodeBase* UPCGEditorGraph::GetEditorNodeFromPCGNode(const UPCGNode* InPCGNode)
{
	return const_cast<UPCGEditorGraphNodeBase*>(static_cast<const UPCGEditorGraph*>(this)->GetEditorNodeFromPCGNode(InPCGNode));
}
...
void UPCGEditorGraph::CreateLinks(UPCGEditorGraphNodeBase* GraphNode, bool bCreateInbound, bool bCreateOutbound, const TMap<UPCGNode*, UPCGEditorGraphNodeBase*>& InPCGNodeToPCGEditorNodeMap)
{
	...
	if (bCreateInbound)
	{
		for (UPCGPin* InputPin : PCGNode->GetInputPins())
		{
			...
			if (!InPin)
			{
				UE_LOGF(LogPCGEditor, Error, "Invalid InputPin for %ls", *InputPin->Properties.Label.ToString());
				ensure(false);
				continue;
			}
			...
			for (const UPCGEdge* InboundEdge : InputPin->Edges)
			{
				...
				{
					UE_LOGF(LogPCGEditor, Error, "Invalid inbound edge for %ls", *InputPin->Properties.Label.ToString());
					ensure(false);
					continue;
				}
				...
				{
					UE_LOGF(LogPCGEditor, Error, "Invalid inbound node for %ls", *InputPin->Properties.Label.ToString());
					continue;
				}
				...
				if (UPCGEditorGraphNodeBase* const* ConnectedGraphNode = InboundNode ? InPCGNodeToPCGEditorNodeMap.Find(InboundNode) : nullptr)
				{
					...
					{
						UE_LOGF(LogPCGEditor, Error, "Could not find inbound pin '%ls' to link to input pin '%ls' from node '%ls'",
	...
	if (bCreateOutbound)
	{
		for (UPCGPin* OutputPin : PCGNode->GetOutputPins())
		{
			...
			if (!OutPin)
			{
				UE_LOGF(LogPCGEditor, Error, "Invalid OutputPin for %ls", *OutputPin->Properties.Label.ToString());
				ensure(false);
				continue;
			}
			...
			for (const UPCGEdge* OutboundEdge : OutputPin->Edges)
			{
				...
				{
					UE_LOGF(LogPCGEditor, Error, "Invalid outbound edge for %ls", *OutputPin->Properties.Label.ToString());
					ensure(false);
					continue;
				}
				...
				{
					UE_LOGF(LogPCGEditor, Error, "Invalid outbound node for %ls", *OutputPin->Properties.Label.ToString());
					continue;
				}
				...
				if (UPCGEditorGraphNodeBase* const* ConnectedGraphNode = OutboundNode ? InPCGNodeToPCGEditorNodeMap.Find(OutboundNode) : nullptr)
				{
					...
					{
						UE_LOGF(LogPCGEditor, Error, "Could not find outbound pin '%ls' to link to input pin '%ls' on node '%ls'",
...
void UPCGEditorGraph::OnGraphChanged(UPCGGraphInterface* InGraph, EPCGChangeType ChangeType)
{
	constexpr EPCGChangeType IgnoredChangeTypes = EPCGChangeType::Cosmetic | EPCGChangeType::Input | EPCGChangeType::ShaderSource | EPCGChangeType::GraphCustomization;
	if (!!(ChangeType & ~IgnoredChangeTypes))
	{
		CachedEdgeUsedByExecution.Reset();

		for (UEdGraphNode* EdGraphNode : Nodes)
		{
			if (UPCGEditorGraphNodeBase* PCGEditorNode = Cast<UPCGEditorGraphNodeBase>(EdGraphNode))
			{
				PCGEditorNode->ClearPinColorCache();
			}
		}
	}

	if (!!(ChangeType & EPCGChangeType::ExternalModification))
	{
		//Changes caused by external tools: Editor graph should be reconstructed.
		ReconstructGraph();
	}
}
...
void UPCGEditorGraph::OnGraphUserParametersChanged(UPCGGraphInterface* InGraph, EPCGGraphParameterEvent ChangeType, FName ChangedPropertyName)
{
	...
	// Those change types can result in a parameter node being removed from the graph. (same condition found in UPCGGraph::OnGraphParametersChanged)
	const bool bCheckForRemovedNodes = ChangeType == EPCGGraphParameterEvent::RemovedUnused
...
bool UPCGEditorGraph::IsEdgeUsedByExecution(const UPCGEdge* InEdge) const
{
	if (const bool* Cached = CachedEdgeUsedByExecution.Find(InEdge))
	{
		return *Cached;
	}

	// IsUsedByExecution dictated by downstream input pin.
	const UPCGNode* Node = InEdge && InEdge->OutputPin ? InEdge->OutputPin->Node : nullptr;
	const bool bUsed = !Node || Node->IsEdgeUsedByNodeExecution(InEdge);
	CachedEdgeUsedByExecution.Add(InEdge, bUsed);
