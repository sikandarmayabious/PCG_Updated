...
/**
* Base class for interactive tool settings in PCG.
* It is useful to derive from this class instead of UPCGInteractiveToolSettings when the tool does not require to create an actor,
* use a graph or a PCG component.
*/
UCLASS(Abstract)
class UPCGInteractiveToolBaseSettings : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
...
public:
	...
	virtual FName GetToolTag() const PURE_VIRTUAL(UPCGInteractiveToolBaseSettings::GetToolTag, return NAME_None;);
};
...
*/
UCLASS(Abstract)
class UPCGInteractiveToolSettings : public UPCGInteractiveToolBaseSettings, public FTickableEditorObject, public IKeyInputBehaviorTarget
{
	GENERATED_BODY()

public:
	UPCGInteractiveToolSettings();
	virtual ~UPCGInteractiveToolSettings() override = default;

	virtual TValueOrError<bool, FText> Initialize(UInteractiveTool* InTool) override;
	virtual void Apply(UInteractiveTool* OwningTool) override;
	virtual void Cancel(UInteractiveTool* OwningTool) override;
	...
	virtual bool CanHandleMultipleActorSelection() const { return false; }
...
UCLASS(Abstract)
class UPCGInteractiveTool : public UInteractiveTool
{
	GENERATED_BODY()

public:
	virtual bool RequiresSettings() const { return true; }
