...
#include "Tools/ManualEdit/PCGManualEditTool.h"
...
/** Widget helper that exposes the standard editor transform gizmo. */
class FPCGEditorModeWidgetHelper : public FLegacyEdModeWidgetHelper
{
public:
	virtual bool ShouldDrawWidget() const override
	{
		return ActiveToolUsesTransformGizmo() && FLegacyEdModeWidgetHelper::ShouldDrawWidget();
	}

	virtual bool UsesTransformWidget() const override
	{
		return ActiveToolUsesTransformGizmo() && FLegacyEdModeWidgetHelper::UsesTransformWidget();
	}

	virtual bool UsesTransformWidget(const UE::Widget::EWidgetMode WidgetMode) const override
	{
		return ActiveToolUsesTransformGizmo() && FLegacyEdModeWidgetHelper::UsesTransformWidget(WidgetMode);
	}

	virtual bool CanCycleWidgetMode() const override
	{
		return ActiveToolUsesTransformGizmo() && FLegacyEdModeWidgetHelper::CanCycleWidgetMode();
	}

	/** Keep the project's ITF TRS gizmo enabled. */
	virtual bool RequiresLegacyViewportInteractions() const override
	{
		return false;
	}

	TWeakObjectPtr<UPCGEditorMode> PCGEditorMode;

private:
	bool ActiveToolUsesTransformGizmo() const
	{
		const UPCGEditorMode* Mode = PCGEditorMode.Get();
		return Mode && Mode->ActiveToolUsesTransformGizmo();
	}
};
...
namespace PCGEditorMode
{
	...
	static FAutoConsoleCommand CommandEnablePCGTool(
		TEXT("pcg.EnableTool"),
		TEXT("Enables toolmode and selects the specified tool."),
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
		{
			GLevelEditorModeTools().ActivateMode(UPCGEditorMode::EM_PCGEditorModeId, /*bToggle=*/false);
			if (UEdMode* ThisMode = GLevelEditorModeTools().GetActiveScriptableMode(UPCGEditorMode::EM_PCGEditorModeId); ThisMode && !Args.IsEmpty())
			{
				ThisMode->GetInteractiveToolsContext()->StartTool(Args[0]);
			}
		}));

	static FAutoConsoleCommand CommandSetGraphInTool(
		TEXT("pcg.tool.SetGraph"),
		TEXT("Sets graph on the current tool."),
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
		{
			UEdMode* ThisMode = GLevelEditorModeTools().GetActiveScriptableMode(UPCGEditorMode::EM_PCGEditorModeId);
			UPCGEditorMode* EditorMode = Cast<UPCGEditorMode>(ThisMode);
			UPCGInteractiveToolSettings* ToolSettings = EditorMode ? Cast<UPCGInteractiveToolSettings>(EditorMode->GetCurrentToolSettings()) : nullptr;
			if (ToolSettings && !Args.IsEmpty())
			{
				// If the input path is empty, or not pointing to a graph, the tool graph will be set to nullptr.
				const FString& GraphPathString = Args[0];
				const FSoftObjectPath GraphPath(GraphPathString);
				UPCGGraphInterface* Graph = Cast<UPCGGraphInterface>(GraphPath.TryLoad());
				ToolSettings->SetToolGraph(Graph);

				if (Args.Num() > 1)
				{
					ToolSettings->SetActorLabel(FName(Args[1]));
				}
			}
		}));

	static FAutoConsoleCommand CommandIsolate(
		TEXT("pcg.tool.Isolate"),
		TEXT("Isolates (like clear PCG link) content from an actor with a PCG component into a separate actor."),
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
		{
			if(Args.IsEmpty())
			{
				UE_LOGF(LogPCGEditor, Error, "Invalid arguments to the pcg.tool.Isolate command.");
				return;
			}

			// [0] - selected actor
			FSoftObjectPath ActorPath(Args[0]);
			UObject* ActorObject = ActorPath.TryLoad();

			AActor* Actor = Cast<AActor>(ActorObject);

			if (!Actor)
			{
				UE_LOGF(LogPCGEditor, Error, "Unable to retrieve the actor '%ls' in the pcg.tool.Isolate command.", *Args[0]);
				return;
			}
			...
			for (int ArgIndex = 1; ArgIndex < Args.Num(); ++ArgIndex)
			{
				Tags.Add(FName(Args[ArgIndex]));
			}
...
void UPCGEditorMode::Enter()
{
	...
	{
		UPCGQueryToolBuilder* QueryToolBuilder = NewObject<UPCGQueryToolBuilder>();
		QueryToolBuilder->SetToolClass(UPCGQueryTool::StaticClass());
		RegisterTool(ToolManagerCommands.EnableQueryTool, UPCGQueryTool::StaticGetToolTag().ToString(), QueryToolBuilder);
	}

	{
		UPCGQueryToolBuilder* QueryToolBuilder = NewObject<UPCGQueryToolBuilder>();
		QueryToolBuilder->SetToolClass(UPCGIsolateTool::StaticClass());
		RegisterTool(ToolManagerCommands.EnableIsolateTool, UPCGIsolateTool::StaticGetToolTag().ToString(), QueryToolBuilder);
	}

	{
		UPCGManualEditToolBuilder* ManualEditToolBuilder = NewObject<UPCGManualEditToolBuilder>();
		RegisterTool(ToolManagerCommands.EnableManualEditTool, UPCGManualEditTool::StaticGetToolTag().ToString(), ManualEditToolBuilder);
	}
...
bool UPCGEditorMode::IsSelectionAllowed(AActor* InActor, bool bInSelection) const
{
	...
	if (const UPCGInteractiveToolBaseSettings* ToolSettings = GetCurrentToolSettings())
	{
...
TSharedRef<FLegacyEdModeWidgetHelper> UPCGEditorMode::CreateWidgetHelper()
{
	TSharedRef<FPCGEditorModeWidgetHelper> Helper = MakeShared<FPCGEditorModeWidgetHelper>();
	Helper->PCGEditorMode = this;

	return Helper;
}

bool UPCGEditorMode::ActiveToolUsesTransformGizmo() const
{
	UInteractiveToolManager* ToolManager = GetToolManager();
	...
	// Generic tool opt-in. Any PCG tool that overrides UsesTransformGizmo() gets the standard transform gizmo.
	const UPCGInteractiveTool* ActiveTool = Cast<UPCGInteractiveTool>(ToolManager->GetActiveTool(EToolSide::Left));
	return ActiveTool && ActiveTool->UsesTransformGizmo();
}
...
UPCGInteractiveToolBaseSettings* UPCGEditorMode::GetCurrentToolSettings() const
{
	UInteractiveToolManager* ToolManager = GetToolManager();
	...
	if (ToolManager && ToolManager->HasAnyActiveTool())
	{
		if (UInteractiveTool* Tool = ToolManager->GetActiveTool(EToolSide::Mouse))
		{
			...
			{
				return Candidate->IsA<UPCGInteractiveToolBaseSettings>();
			});

			if (FoundProperties)
			{
				return Cast<UPCGInteractiveToolBaseSettings>(*FoundProperties);
			}
		}
	}
