...
namespace PCGEditorGraphNodePin::Constants
{
	static constexpr int32 MaxPinLabelLength = 128;
}
...
void SPCGEditorGraphNodePin::Construct(const FArguments& InArgs, UEdGraphPin* InPin)
{
	...
	const FSlateColor PinColor = GetPinColor();
	const FSlateColor PinTextColor = GetPinTextColor();
	...
	TSharedRef<SWidget> PinWidgetRef =
		SNew(SLayeredImage, GetSecondaryPinIcon(), GetSecondaryPinColor())
		.Image(MakeAttributeSP(this, &SPCGEditorGraphNodePin::GetPinIcon))
		.ColorAndOpacity(PinColor);
	...
	if (InArgs._IsRenameable)
	{
		SAssignNew(InlineEditableLabel, SInlineEditableTextBlock)
			.Text(GetPinLabel())
			.Style(FPCGEditorStyle::Get(), "PCG.Node.NodeTitleInlineEditableText")
			.Visibility(InArgs._ShowLabel ? EVisibility::Visible : EVisibility::Collapsed)
			.ColorAndOpacity(PinTextColor)
			.OnVerifyTextChanged(this, &SPCGEditorGraphNodePin::OnVerifyPinLabelChanged)
			.OnTextCommitted(this, &SPCGEditorGraphNodePin::OnPinLabelCommitted)
			.IsReadOnly(false)
			.MultiLine(false);

		LabelWidget = InlineEditableLabel.ToSharedRef();
	}
	else
	{
	...
	else
	{
		...
		if (ValueWidget != SNullWidget::NullWidget)
		{
			...
			{
				ValueBox->SetEnabled(IsEditingEnabled());
			}
		}
	...
	SBorder::Construct(SBorder::FArguments()
		.BorderImage(this, &SPCGEditorGraphNodePin::GetPinBorder) // Requires dynamic lambda
		.BorderBackgroundColor(this, &SPCGEditorGraphNodePin::GetHighlightColor) // Requires dynamic lambda
		.OnMouseButtonDown(this, &SPCGEditorGraphNodePin::OnPinNameMouseDown) // Requires dynamic lambda
		.Padding(0) // NOTE: This is different from base class implementation
		[
			SNew(SBorder)
			.BorderImage(CachedImg_Pin_DiffOutline)
			.BorderBackgroundColor(this, &SPCGEditorGraphNodePin::GetPinDiffColor) // Requires dynamic lambda
			.Padding(0) // NOTE: This is different from base class implementation
			[
				SNew(SLevelOfDetailBranchNode)
				.UseLowDetailSlot(this, &SPCGEditorGraphNodePin::UseLowDetailPinNames) // Requires dynamic lambda
				.LowDetail()
				[
...
bool SPCGEditorGraphNodePin::SetPinDefaultValue(const FString& NewValue)
{
	if (!GraphPinObj || GraphPinObj->IsPendingKill() || GraphPinObj->DefaultValue == NewValue)
	{
		return false;
	}
	...
	const FScopedTransaction Transaction(NSLOCTEXT("PCGGraphEditor", "ChangePinDefaultValue", "Change Pin Default Value"));
	GraphPinObj->Modify();
...
void SPCGEditorGraphNodePin::EnterLabelEditMode()
{
	if (InlineEditableLabel.IsValid())
	{
		InlineEditableLabel->EnterEditingMode();
	}
}

bool SPCGEditorGraphNodePin::OnVerifyPinLabelChanged(const FText& InText, FText& OutErrorMessage)
{
	using namespace PCGEditorGraphNodePin;

	const FString NewName = InText.ToString();

	if (NewName.IsEmpty())
	{
		OutErrorMessage = NSLOCTEXT("PCGGraphEditor", "EmptyPinName", "Pin name cannot be empty.");
		return false;
	}

	if (NewName.Len() > Constants::MaxPinLabelLength)
	{
		OutErrorMessage = FText::Format(NSLOCTEXT("PCGGraphEditor", "PinNameTooLong", "Pin name cannot exceed {0} characters."), Constants::MaxPinLabelLength);
		return false;
	}

	// Reject invalid characters using UE's standard name validation
	if (!FName::IsValidXName(NewName, INVALID_OBJECTNAME_CHARACTERS, &OutErrorMessage))
	{
		return false;
	}

	// Reject duplicates among sibling pins in the same direction
	if (const UEdGraphPin* Pin = GetPinObj())
	{
		if (const UEdGraphNode* Node = Pin->GetOwningNode())
		{
			const FName NewLabel(*NewName);
			for (const UEdGraphPin* OtherPin : Node->GetAllPins())
			{
				if (OtherPin != Pin && OtherPin->Direction == Pin->Direction && OtherPin->PinName == NewLabel)
				{
					OutErrorMessage = NSLOCTEXT("PCGGraphEditor", "DuplicatePinName", "A pin with this name already exists.");
					return false;
				}
			}
		}
...
void SPCGEditorGraphNodePin::OnPinLabelCommitted(const FText& InText, ETextCommit::Type CommitType)
{
	...
	if (!Pin || InText.IsEmpty())
	{
		return;
	}

	const FName NewLabel(*InText.ToString());
	if (NewLabel == Pin->PinName)
	{
		return;
	}

	if (UPCGEditorGraphNodeBase* EditorNode = Cast<UPCGEditorGraphNodeBase>(Pin->GetOwningNode()))
	{
		EditorNode->OnUserRenameDynamicPin(Pin, NewLabel);
	}
}
