...
bool UPCGToolset::SetGraphParams(UPCGGraph* Graph, const TArray<FPCGParamDefinition>& Params)
{
	...
	for (const FPCGParamDefinition& Param : Params)
	{
		...
		if (Param.ValueTypeObject) NewDesc.ValueTypeObject = Param.ValueTypeObject;
...
bool UPCGToolset::SetGraphParameterCategories(UPCGGraph* Graph, const TMap<FName, FString>& Categories)
{
	if (!Graph || !Graph->GetUserParametersStruct()->GetPropertyBagStruct()) return false;
	UPropertyBagHierarchyRoot* Root = Cast<UPropertyBagHierarchyRoot>(Graph->GetUserParameterHierarchyRoot());
	if (!Root) return false;
	const auto Existing = Graph->GetUserParametersStruct()->GetPropertyBagStruct()->GetPropertyDescs();
	for (const auto& Pair : Categories)
	{
		if (!Graph->GetUserParametersStruct()->FindPropertyDescByName(Pair.Key)) return false;
	}
	TArray<FPropertyBagPropertyDesc> Descs(Existing);
	for (FPropertyBagPropertyDesc& Desc : Descs)
	{
		if (const FString* Category = Categories.Find(Desc.Name)) Desc.SetMetaData(TEXT("Category"), *Category);
	}
	Graph->Modify();
	Graph->UpdateUserParametersStruct([&Descs](FInstancedPropertyBag& Bag)
	{
		Bag.MigrateToNewBagStruct(UPropertyBag::GetOrCreateFromDescs(Descs));
	});
	// UE 5.8 renders the saved hierarchy rather than descriptor categories alone.
	Root->Modify();
	Root->GetChildrenMutable().Reset();
	GetDefault<UPropertyBagSchema>()->TransferPropertyBagMetadataIntoHierarchy(*Graph->GetUserParametersStruct(), *Root);
	Graph->OnGraphParametersChanged(EPCGGraphParameterEvent::CategoryChanged, TEXT("UserParameters"));
...
TArray<FPCGGraphInstanceInfo> UPCGToolset::ListGraphInstances()
{
	...
	for (TActorIterator<APCGVolume> It(World); It; ++It)
	{
		...
		if (!PCGVolume || !PCGVolume->FindComponentByClass<UPCGComponent>())
		{
			continue;
		}

		if (UPCGGraphInstance* GraphInstance = PCGVolume->FindComponentByClass<UPCGComponent>()->GetGraphInstance())
		{
			FPCGGraphInstanceInfo Info;
			Info.Actor = PCGVolume;
			Info.Graph = PCGVolume->FindComponentByClass<UPCGComponent>()->GetGraph();
			Result.Add(Info);
		}
	}
	return Result;
...
FPCGGraphInstanceInfo UPCGToolset::SpawnGraphInstance(UPCGGraph* Graph, const FString& Name, const FTransform& Transform, const FString& JsonParams)
{
	...
	UPCGComponent* Component = PCGVolume->FindComponentByClass<UPCGComponent>();
	if (!Component)
	{
...
FInstancedPropertyBag UPCGToolset::GetGraphInstanceParams(const AActor* PCGVolume)
{
	using namespace PCGToolsetLibrary;
	if (!PCGVolume || !PCGVolume->FindComponentByClass<UPCGComponent>() || !PCGVolume->FindComponentByClass<UPCGComponent>()->GetGraphInstance())
	{
		UKismetSystemLibrary::RaiseScriptError(TEXT("Error: PCGVolume is invalid"));
		return {};
	}

	const FInstancedPropertyBag& PropertyBag = PCGVolume->FindComponentByClass<UPCGComponent>()->GetGraphInstance()->ParametersOverrides.Parameters;
...
bool UPCGToolset::SetGraphInstanceParams(const AActor* PCGVolume, const FString& JsonParams)
{
	using namespace PCGToolsetLibrary;
	if (!PCGVolume)
	{
	...
	if (!PCGVolume->FindComponentByClass<UPCGComponent>())
	{
		UKismetSystemLibrary::RaiseScriptError("Error: PCGVolume has no PCG component");
		return false;
	}

	UPCGGraphInstance* InOutGraphInstance = PCGVolume->FindComponentByClass<UPCGComponent>()->GetGraphInstance();
...
bool UPCGToolset::ResetGraphInstanceParams(const AActor* PCGVolume, const TArray<FName>& ParamNames)
{
	if (!PCGVolume || !PCGVolume->FindComponentByClass<UPCGComponent>() || !PCGVolume->FindComponentByClass<UPCGComponent>()->GetGraphInstance())
	{
		UKismetSystemLibrary::RaiseScriptError(TEXT("Error: PCGVolume is invalid"));
	...
	UPCGGraphInstance* GraphInstance = PCGVolume->FindComponentByClass<UPCGComponent>()->GetGraphInstance();
	...
	const int32 YPositionIdx)
{
	...
	// Project and third-party settings are not in the built-in PCG package catalogue.
	UClass* SettingsClass = FoundSettings ? (*FoundSettings)->GetClass() : nullptr;
	if (!SettingsClass && NativeNodeType.StartsWith(TEXT("/Script/")))
	{
		SettingsClass = LoadClass<UPCGSettings>(nullptr, *NativeNodeType);
	...
	UPCGNode* CreatedNode = Graph->AddNodeOfType(SettingsClass, DefaultSettings);
	if (!CreatedNode)
	{
		Transaction.Cancel();
