...
bool PCGToolsetLibrary::Graph::SetGraphInstanceParams(UPCGGraphInstance* GraphInstance, const FString& JsonParams)
{
	...
	const bool bSuccess = UToolsetLibrary::SetObjectProperties(GraphInstance, Json::ToJsonString(Wrapped), EBypassContainerCheck::Yes);
	if (bSuccess)
	{
		// Nested property-bag writes do not reliably emit a parameter event. Invalidate
		// the graph execution cache after the new values, not just the override flags.
		for (const auto& Pair : ParsedJson->Values)
			GraphInstance->OnGraphParametersChanged(EPCGGraphParameterEvent::ValueModifiedLocally, FName(*Pair.Key));
	}
	return bSuccess;
}
